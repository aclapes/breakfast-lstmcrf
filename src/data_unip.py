...
class VideoGenerator(object):
    ...
    def next(self):
        ...
        if vid_array is not None:
            ...
            nb_instances = nb_frames // (self.length//2)
            vid_array = vid_array[:nb_instances * (self.length//2), :, :, :]
            vid_array = vid_array.reshape((nb_instances, (self.length//2), 3, ) + (
                self.input_size))
            vid_array = np.concatenate([vid_array[:-1],vid_array[1:]],axis=1)
