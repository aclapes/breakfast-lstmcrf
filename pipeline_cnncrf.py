...
class SimpleCnnCrfModel(object):
...
class SimpleCnnCrfPipeline(object):
    def __init__(self,
                 train,
                 val,
                 te,
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('Model', reuse=False, initializer=initializer):
                self.train_model = SimpleCnnCrfModel(config=config, input_data=train, is_training=True)

            with tf.variable_scope('Model', reuse=True, initializer=initializer):
                self.val_model = SimpleCnnCrfModel(config=config, input_data=val, is_training=False)

            with tf.variable_scope('Model', reuse=True, initializer=initializer):
                self.te_model = SimpleCnnCrfModel(config=test_config, input_data=te, is_training=False)
