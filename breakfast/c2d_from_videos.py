...
def create(info_file, labels_file, frame_size, action_labels, output_dir, fskip=5):
    ...
    #     vid_lbl = basename(splitext(videos_data[key]['url'])[0]).split('_')[1]
    #     activities.setdefault(vid_lbl, 0)
    #     activities[vid_lbl] += 1
    #
    # print activities
    ...
    for subset in subsets:
        ...
        # yres = []
        # for key in videos_data.keys():
        #     x_str,y_str = videos_data[key]['resolution'].split('x')
        #     xres.append(int(x_str))
        #     yres.append(int(y_str))
        # print np.unique(xres, return_counts=True)
        # print np.unique(yres, return_counts=True)
        #
        # continue
        ...
        for i,key in enumerate(videos):
            ...
            if action_labels:
                ...
                y = labels.index(basename(splitext(videos_data[key]['url'])[0]).split('_')[1])
            ...
            for j in range(nr_samples_per_vid):
                im = X[j*fskip]
                lbl = Y[j*fskip] if action_labels else y
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        'If True action labels are used, if not, activities (video level annotations) (default: %(default)s)')
    ...
    parser.add_argument(
        ...
        type=str,
        dest='labels',
        default='breakfast/activity_labels.txt',
        help=
