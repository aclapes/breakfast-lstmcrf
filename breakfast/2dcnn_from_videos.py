...
def create(info_file, labels_file, frameskip, frame_size, output_dir):
    ...
    for subset in subsets:
        ...
        nr_frames_sb = np.sum([videos_data[key]['num_frames'] for key in videos])
        n = nr_frames_sb // frameskip

        output_file = join(output_dir, subset + '.h5')
        f_dataset = h5py.File(output_file, 'w')

        f_dataset.create_dataset('video_features',(n,height,width,3), chunks=(1,224,224,3), dtype=np.uint8, compression='gzip', compression_opts=9)
        f_dataset.create_dataset('outputs', (n,1), dtype=np.uint8)
        ...
        for i,key in enumerate(videos):
            ...
            x = vid_to_array(videos_data[key]['url'], frame_size=(height,width))[::frameskip]
            y = generate_output(videos_data[key], labels, length=1)[::frameskip]

            for k,(im,lbl) in enumerate(zip(x,y)):
                f_dataset['video_features'][perm[k]] = im
                f_dataset['outputs'][perm[k]] = lbl
                class_counts[lbl] += 1
