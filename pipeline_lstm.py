...
class SimpleLstmModel(object):
    def __init__(self, config, input_data, test_subset, summaries_dir, is_training=False):
        ...
        with tf.name_scope('evaluation'):
            self.predictions, _ = tf.arg_max(logits, axis=-1)
        ...
        with self.graph.as_default():
            with tf.name_scope('Train'):
                with tf.variable_scope('Model', initializer=tf.random_uniform_initializer(-0.01, 0.01)):
                    self.train_model = SimpleLstmModel(config=config, input_data=input_data,
                                                          test_subset=test_subset,
                                                          summaries_dir=path.join(logging_path, 'train'),
                                                          is_training=True)
            with tf.name_scope('Validation'):
                with tf.variable_scope('Model', reuse=True):
                    self.val_model = SimpleLstmModel(config=val_config, input_data=input_data,
                                                        test_subset=test_subset,
                                                        summaries_dir=path.join(logging_path, 'validation'),
                                                        is_training=False)
            with tf.name_scope('Test'):
                with tf.variable_scope('Model', reuse=True):
                    self.te_model = SimpleLstmModel(config=test_config, input_data=input_data,
                                                       test_subset=test_subset,
